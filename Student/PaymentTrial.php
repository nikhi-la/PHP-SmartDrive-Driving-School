...
if($_SESSION["trialvehicletypeid"]="VT2022110001")
{
	$trialamount=150;
}
 if($_SESSION["trialvehicletypeid"]="VT2022110002")
{
	$trialamount=250;
} 

if($_SESSION["trialvehicletypeid"]="VT2022110003")
{
	$trialamount=400;
} 
if($_SESSION["trialvehicletypeid"]="VT2022110004")
{
	$trialamount=350;
}
$name=$_POST["txtname"];
...
else
{
	...
	$batchid=$data3["batch_id"];  
 	...
 	$bookeddate=$data3["studentslot_date"];  
$updateQry="update tbl_studentslot set slotbook_status=1 where studentslot_id= '".$_SESSION["studentslotid"]."'";
	$con->query($updateQry);
...
$zero="";
$traid=$reg.$zero.$trid;
$upQry="update tbl_regid set tid_number='".$trid."' where id=1";
$con->query($upQry);

	$insQry1="insert into tbl_trial(trial_id,vehicletype_id,booked_date,booking_date,studentslot_id,trial_amount,payment_status,student_id,emp_id)values('".$traid."','".$_SESSION["trialvehicletypeid"]."',curdate(),'".$bookeddate."','".$_SESSION["studentslotid"]."','".$trialamount."',1,'".$_SESSION["uid"]."','".$_SESSION["emp"]."')";
...

{
   $mail = new PHPMailer(true);
    ...
    $mail->Body = "Hello"." ".ucfirst($data["student_name"])." ".ucfirst($data["student_middle"])." ".ucfirst($data["student_last"])." "."Your payment for trial run  has been completed";
  if($mail->send())
  {
    echo "Sended";
  }
...
<script>
alert("Slot Booked");
window.location="TrialRunSlot.php";
</script>
<?php	
}
}
