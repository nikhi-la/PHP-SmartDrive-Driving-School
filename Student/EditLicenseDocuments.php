...
$update="update tbl_license set license_photo='".$photo."',license_signature='".$signature."',license_permanentap='".$paddressproof."',license_self='".$selfdeclaration."',license_ageproof='".$ageproof."',license_presentap='".$addressproof."',license_eyetest='".$eyecertificate."',cur_date=curdate() where license_id='".$_GET["eid"]."' and license_status=1";
if($con->query($update))
{
	?>
    <script>
	alert("Data Inserted");
	location.href="LicenseDocuments.php";
	</script>
...
<h2>Licence Documents</h2>
  ...
  <table width="723" height="645" border="1" cellpadding="10">
    <tr>
      ...
      <td width="387"><input type="file" name="photo" id="photo" onchange="OnPhotoValidation()" required="required" autocomplete="off"/><g>.jpg,.jpeg(size between 10kb - 20 kb)</g><span id="photocss"></span></div></td>
    </tr>
    <tr>
      <th>Signature</th>
      <td><input type="file" name="signature" id="signature" onchange="SignatureValidation()" required="required" autocomplete="off"/><g>.jpg,.jpeg(size between 10kb - 20 kb)</g><span id="signaturecss"></span></div></td>
    </tr>
    <tr>
