...
if(isset($_GET["did"]))
{
$sel="select * from  tbl_studentdeparture where student_id='".$_GET["did"]."'";
$row1=$con->query($sel);
...
if($con->query($insQry))
{
	...
	<script>
	alert("student  Added");
	location.href="ViewDeparturedstudent.php";
	</script>
<?php
...
<?php
 ...
 $delQry="delete from tbl_studentdeparture where student_id='".$_GET["did"]."'";
 $con->query($delQry);
}
?>
<br><br><br><br>
...
<form id="form1" name="form1" method="post" action="studentloyeeList.php">
  ...
  <table width="1227" height="98" border="1" cellpadding="10">
    ...
    <?php
    $selQry="select * from tbl_studentdeparture e inner join tbl_district d on e.district_id=d.district_id ";
	...
	<td><center><a href="../Assets/Files/studentProof/<?php echo $data["student_proof"];?>" download>
     <i class="fa fa-download " style="font-size:48px;cursor: pointer;color:black;"></i></center></td>
      <td><?php echo $data["student_doj"];?></td>
      <td><?php echo $data["student_departuredate"];?></td>
      <td><a href="ViewDeparturedstudent.php?did=<?php echo $data["student_id"];?>">Add Back</a></td>
    </tr>
    <?php
	}
	?>
