...
<p>&nbsp;</p>
  ...
  <div id="pri">
	...
	$sel="select * from  tbl_slotbooking sb inner join tbl_student s inner join tbl_studentslot ss inner join tbl_batch b inner join tbl_employee e on sb.student_id=s.student_id and sb.studentslot_id= ss.studentslot_id and ss.batch_id=b.batch_id and b.emp_id=e.emp_id where  sb.student_id='".$studentid."'";
	
	$sel1= $sel ."  order by sb.booking_date,studentslot_number";
	
	if($_POST["txtfrom"]!="" && $_POST["txtto"]!="")
	{
		$sel1 = $sel ."  and sb.booking_date between '".$_POST["txtfrom"]."' and '".$_POST["txtto"]."' order by booking_date,studentslot_number";
	}
	if($_POST["txtfrom"]!="" && $_POST["txtto"]=="")
	{
		$sel1 = $sel ."  and sb.booking_date between '".$_POST["txtfrom"]."' and '".$max."' order by booking_date,studentslot_number";
	}
	if($_POST["txtto"]!="" && $_POST["txtfrom"]=="")
	{
		$sel1 = $sel ." and sb.booking_date <= '".$_POST["txtto"]."' order by booking_date,studentslot_number";
	}
	$row=$con->query($sel1);
	$i=0;
	while($data=$row->fetch_assoc())
