...
{
		$setqry="update tbl_salary set paid_status=1 , paid_date=curdate() where salary_id='".$_SESSION["sid"]."'";
		  if($con->query($setqry))
	{
		
		$sel="select * from tbl_employee e inner join tbl_salary s on e.emp_id=s.emp_id where s.salary_id='".$_SESSION["sid"]."'";
		$row=$con->query($sel);
    ...
    $mail->addAddress($data["emp_email"]);
  
    $mail->isHTML(true);
  
    $mail->Subject = "Salary Credited ";
    $mail->Body = "Hello"." ".ucfirst($data["emp_name"])." ".ucfirst($data["emp_middle"])." ".ucfirst($data["emp_last"])." "."Your salary has been credited";
  if($mail->send())
  {
  }
  else
  ...
  <div class="payment">
    ...
    <div class="form">
      ...
      <div class="card space icon-relative">
        ...
        <input type="text" name="txtcard" class="input"  placeholder="Card Number" autocomplete="off" required="required" pattern="[0-9]{16}"  title="16 digit with 0-9">
