...
if(isset($_GET["aid"]))
{
$sel1="select * from tbl_student  where student_id='".$_GET["aid"]."'";
$row2=$con->query($sel1);
...
$con->query($updateQry);
    ...
    $mail->Body = "Hello"." ".ucfirst($data2["student_name"])." ".ucfirst($data2["student_middle"])." ".ucfirst($data2["student_last"])." ".".Your smartdrive account has been activated.You can login with your email and password.Thank you.";
  if($mail->send())
  {
    echo "Sended";
  }
  else
  {
	...
	?>
    <script>
	window.location="RejectedStudent.php";
	</script>
    <?php  
}
?>
...
<form id="form1" name="form1" method="post" action="RejectedStudent.php">
  <h1>&nbsp;</h1>
  <h1>Rejected Students List</h1>
  <p>&nbsp;</p>
  <table  border="1" cellpadding="10">
    ...
    <?php
    $selQry="select * from tbl_student s inner join tbl_district d on s.district_id=d.district_id where student_vstatus=2";
	...
	<tr >
     ...
     <i class="fa fa-download " style="font-size:48px;cursor: pointer;color:black;"></i></center></td>
      <td width="81">&nbsp;<a href="RejectedStudent.php?aid=<?php echo $data["student_id"];?>">Accept</a></td>
    </tr>
    <?php
	}
	?>
