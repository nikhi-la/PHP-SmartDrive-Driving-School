...
$row1=$con->query($sel);
$datas=$row1->fetch_assoc();

$reg=$datas["sreg_id"];
$sid=$datas["sid_number"];
$sid++;
$length=strlen($sid);
if($length==1)
$zero="000";
...
if($length==4)
$zero="";
$studentid=$reg.$zero.$sid;
$upQry="update tbl_regid set sid_number='".$sid."' where id=1";
$con->query($upQry);

$insQry="insert into tbl_student(student_id,student_name,student_middle,student_last,student_dob,student_gender,district_id,student_place,student_landmark,student_housename,student_pincode,student_email,student_contact,student_photo,student_proof,student_password,student_doj)values('".$studentid."','".$name."','".$middle."','".$last."','".$dob."','".$gender."','".$district."','".$place."','".$landmark."','".$housename."','".$pincode."','".$email."','".$contact."','".$photo."','".$proof."','".$password."',curdate())";
if($con->query($insQry))
{
    ...
    $mail->Subject = "Registration Progressing";
    $mail->Body = "Hello"." ".ucfirst($_POST["txtname"])." ".ucfirst($_POST["txtmiddle"])." ".ucfirst($_POST["txtlast"])." "."Your registration is under verification.You can login only after verification has been completed.Thank You.";
  if($mail->send())
  {
    echo "";
  }
...
<form id="form1" name="form1" method="post" action="" enctype="multipart/form-data" >
  ...
  <tr>
    ...
    <td colspan="2"><label for="txtdob"></label>
      <input type="date" name="txtdob" id="txtdob"   max=<?php echo date('Y-m-d', strtotime('-6574 days'));?>  min=<?php echo date('Y-m-d', strtotime('-27393 days'));?>   required="required" autocomplete="off"/>
      <span id="mydateError"></span></td>
    </tr>
  <tr>
    <td>Gender<span class="required">  *</span></td>
    <td colspan="2"><div class="input-group">
      <input type="radio" name="txtgender" id="txtgender" onchange="validateGender(this)"  value="Male"  autocomplete="off" required="required"/>
       &nbsp;Male&nbsp;&nbsp;&nbsp;&nbsp;
      <input type="radio" name="txtgender" id="txtgender" onchange="validateGender(this)"  value="Female" autocomplete="off" required="required"/>
      &nbsp; Female&nbsp;&nbsp;&nbsp;&nbsp;
      <input type="radio" name="txtgender" id="txtgender" onchange="validateGender(this)"  value="Others" autocomplete="off" required="required"/>
 ...
 <tr>
  ...
  <tr>
    ...
    <td colspan="2"><div class="input-group">
      ...
      <input type="password" name="cpassword" id="cpassword" onkeyup="validateConfirmPassword()" required="required" autocomplete="off"/>
