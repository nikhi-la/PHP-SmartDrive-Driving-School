...
include("Head.php");
	...
	{
		$sel="select * from tbl_studentattendance where student_id='".$addattendance[$i]."' and attendance_date='".$_POST["txtdate"]."'";
		$row=$con->query($sel);
		if($addattendancedata=$row->fetch_assoc())
		{
			$update="update tbl_studentattendance set attendance_status= '".$attendancep."' where student_id='".$addattendance[$i]."' and attendance_date='".$_POST["txtdate"]."'";
			$con->query($update);
		}
		else
		{
		...
		$ins="insert into tbl_studentattendance(attendance_id,student_id,attendance_date,attendance_status)values('".$sattendid."','".$addattendance[$i]."','".$_POST["txtdate"]."','".$attendancep."')";
		if($con->query($ins))
		{
		?>
		<script>
...
$date=$_POST["txtdate"];
	...
	{
		$sel="select * from tbl_studentattendance where student_id='".$addattendancep[$i]."' and attendance_date='".$_POST["txtdate"]."'";
		$row=$con->query($sel);
		if($addattendancedatap=$row->fetch_assoc())
		{
			$update="update tbl_studentattendance set attendance_status= '".$attendancea."' where student_id='".$addattendancep[$i]."' and attendance_date='".$_POST["txtdate"]."'";
			$con->query($update);
		}
		else
		{
		...
		$ins="insert into tbl_studentattendance(attendance_id,student_id,attendance_date,attendance_status)values('".$sattendid."','".$addattendancep[$i]."','".$_POST["txtdate"]."','".$attendancea."')";
		if($con->query($ins))
		{
		?>
		<script>
...
<center>
  <table width="509" height="49"  border="0" cellpadding="10">
    <tr>
      <td width="108" >Select Date<span class="required">  *</span></td>
...
function getdateSearch()
{
  ...
  //alert(did);
	$.ajax({
	url: "../Assets/AjaxPages/AjaxPreviousAttendanceSearchE.php?d="+d,
